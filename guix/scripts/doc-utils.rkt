...
(define (make-provenance-section #:arch+os [arch+os #f])
  ...
  (match-define (and (list `(list (channel
                                   ,@(list-no-order `(name 'guix)
                                                    `(url ,guix-url)
                                                    `(branch ,guix-branch)
                                                    `(commit ,guix-commit)
                                                    `(introduction
                                                      (make-channel-introduction
                                                       ,guix-start-commit
                                                       (openpgp-fingerprint ,guix-fingerprint))))))
                           built-by built-on packed-by packed-on)
                     (list _ (? non-empty-string?) ...))
    (let ([specs '(["built-by.txt" "HappySome"]
                   ["built-on.txt" "Athens"]
                   ["packed-by.txt" "OtherSome"]
                   ["packed-on.txt" "Stratford"])])
      (if (or RKT_NOT_A_DRILL
          ...
          (cons '(list (channel
                        (name 'guix)
                        (url "https://localhost/guix.git")
                        (branch "NONE")
                        (commit "fake-commit")
                        (introduction (make-channel-introduction
                                       "fake-start-commit" (openpgp-fingerprint "no-fingerprint")))))
                (map cadr specs)))))
  (define (guix-cgit-link rel . text)
    (cond
      [(equal? guix-url "https://git.savannah.gnu.org/git/guix.git")
       (apply hyperlink
              (++ "https://git.savannah.gnu.org/cgit/guix.git/" rel)
              text)]
      [else
       ;; don't know how, in general
       (apply elem text)]))
  (define (guix-commit-link commit)
    @guix-cgit-link[(++ "tree/?id=" commit)]{@tt[commit]})
  @list{
 ...
 @(if arch+os
      @para{The shared library was compiled by @|built-by|
  on @tt[built-on] for @tt[arch+os], and this package was
  assembled by @|packed-by| on @tt[packed-on].}
      @para{The flake was built by @|built-by| on
  @tt[built-on], and this package,
  @make-pkg-name*{native-libs}, was generated by @|packed-by|
  on @tt[packed-on].})
 ...
 @tt{@|nixpkgs.narHash|}.
 Guix was used from the @guix-cgit-link[(++ "log/?h=" guix-branch)]{@tt[guix-branch]}
 branch of @url[guix-url], which resolved to commit @guix-commit-link[guix-commit].
 The Guix channel was authenticated starting at commit
 @guix-commit-link[guix-start-commit] with OpenPGP fingerprint @tt[guix-fingerprint].

 For further details, see the @rel-link{provenance/} directory.
 })

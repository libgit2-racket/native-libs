...
(define (make-provenance-section #:arch+os [arch+os #f])
  ...
  (match-define `(list (channel
    ...
    (if (or RKT_NOT_A_DRILL
            (file-exists? "channels.txt"))  ;; <- extension is to placate `raco setup`
        (file->value "channels.txt")
        '(list (channel
                (name 'guix)
 ...
 @(if arch+os
  ...
  @tt[@make-pkg-name*{native-libs}], was generated by @|packed-by|
  on @tt[packed-on].})
 ...
 For further details, see @rel-link{provenance.rktd}.
 })
